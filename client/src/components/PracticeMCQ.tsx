...
export default function PracticeMCQ({ question, options, correctAnswer, explanation, questionId, onAnswer }: PracticeMCQProps) {
  ...
  return (
    <div className="space-y-4" role="group" aria-labelledby={`mcq-question-${questionId}`}>
      <p className="text-lg font-medium text-foreground" id={`mcq-question-${questionId}`}>{question}</p>
      
      <div className="grid gap-3" role="radiogroup" aria-labelledby={`mcq-question-${questionId}`}>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                {showCorrect && <CheckCircle2 className="w-5 h-5 text-green-600" aria-label="Correct" />}
                {showIncorrect && <XCircle className="w-5 h-5 text-red-600" aria-label="Incorrect" />}
              </div>
            </button>
          );
        })}
      ...
      {!showResult ? (
        <Button
          ...
          aria-label="Vérifier la réponse sélectionnée"
        >
          Vérifier
        </Button>
      ) : (
